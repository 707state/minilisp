...
static inline Obj *forward(Obj *obj)
{
  ...
  // object.
  if (obj->type == TMOVED) return obj->moved;
...
{
  while (isdigit(peek(in))) val = val * 10 + (fgetc(in) - '0');
  return val;
}
...
static Obj *read_expr(void *root, FILE *in)
{
  for (;;) {
    ...
    if (c == '(') return read_list(root, in);
    if (c == ')') return Cparen;
    if (c == '.') return Dot;
    if (c == '\'') return read_quote(root, in);
    if (isdigit(c)) return make_int(root, read_number(c - '0', in));
    if (c == '-' && isdigit(peek(in)))
      return make_int(root, -read_number(0, in));
    if (isalpha(c) || strchr(symbol_chars, c)) return read_symbol(root, c, in);
...
{
  int len = 0;
  for (; list->type == TCELL; list = list->cdr) len++;
...
{
  if (length(*list) != 1) error("Malformed macroexpand");
  DEFINE1(body);
  *body = (*list)->car;
