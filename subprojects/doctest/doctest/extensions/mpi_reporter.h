 ...
 */
    ...
    public:
        ...
        void test_run_end(const TestRunStats& p) override {
            ...
            MPI_Reduce(&p.numAssertsFailed, &g_numAssertsFailed, 1, MPI_INT, MPI_SUM, 0,
                       MPI_COMM_WORLD);
            MPI_Reduce(&p.numTestCasesFailed, &g_numTestCasesFailed, 1, MPI_INT, MPI_SUM, 0,
                       MPI_COMM_WORLD);
            ...
            MPI_Gather(&p.numAssertsFailed, 1, MPI_INT, numAssertsFailedByRank.data(), 1, MPI_INT,
                       0, MPI_COMM_WORLD);

            if(rank == 0) {
                separator_to_stream();
                s << Color::Cyan << "[doctest] " << Color::None
                  << "assertions on all processes: " << std::setw(6) << g_numAsserts << " | "
                ...
                if(nb_test_cases_skipped_insufficient_procs > 0) {
                    ...
                    if(nb_test_cases_skipped_insufficient_procs > 1) {
                        s << nb_test_cases_skipped_insufficient_procs
                          << " tests requiring more than ";
                    } else {
                        s << nb_test_cases_skipped_insufficient_procs
                          << " test requiring more than ";
                    }
                ...
                if(g_numAssertsFailed > 0) {
                    s << Color::Cyan << "[doctest] " << Color::None
                      << "fail on rank:" << std::setw(6) << "\n";
                    for(std::size_t i = 0; i < numAssertsFailedByRank.size(); ++i) {
                        if(numAssertsFailedByRank[i] > 0) {
                            s << std::setw(16) << " -> On rank [" << i << "] with "
                              << numAssertsFailedByRank[i] << " test failed" << std::endl;
                        }
                    }
                }
                s << Color::Cyan << "[doctest] " << Color::None
                  << "Status: " << (g_numTestCasesFailed > 0 ? Color::Red : Color::Green)
                  << ((g_numTestCasesFailed > 0) ? "FAILURE!" : "SUCCESS!") << Color::None
                  << std::endl;
            }
        }
        ...
        void test_case_end(const CurrentTestCaseStats& st) override {
            if(is_mpi_test_case()) {
                ...
                for(const std::pair<std::string, int>& failure : m_failure_str_queue) {
                    ...
                    MPI_Isend(failure_str.c_str(), failure_msg_size, MPI_BYTE, 0, failure_line,
                              MPI_COMM_WORLD, &requests.back()); // Tag = file line
                }
                ...
                MPI_Reduce(&nb_fail_asserts, &nb_fail_asserts_glob, 1, MPI_INT, MPI_SUM, 0,
                           MPI_COMM_WORLD);
                ...
                if(rank == 0) {
                    ...
                    for(std::size_t i = 0; i < static_cast<std::size_t>(nb_fail_asserts_glob);
                        ++i) {
                        ...
                        void*       recv_msg_data = const_cast<char*>(
                                recv_msg.data()); // const_cast needed. Non-const .data() exists in C++11 though...
                        MPI_Recv(recv_msg_data, count, MPI_BYTE, status.MPI_SOURCE, status.MPI_TAG,
                                 MPI_COMM_WORLD, &status_recv);
        ...
        void log_assert(const AssertData& rb) override {
            if(!is_mpi_test_case()) {
                ...
                if((rb.m_at & (assertType::is_throws_as | assertType::is_throws_with)) == 0) {
                    failure_msg << Color::Cyan << assertString(rb.m_at) << "( " << rb.m_expr
                                << " ) " << Color::None

                                << (!rb.m_failed ? "is correct!\n" : "is NOT correct!\n")
                                << "  values: " << assertString(rb.m_at) << "( "
                                << rb.m_decomp.c_str() << " )\n";
                }
